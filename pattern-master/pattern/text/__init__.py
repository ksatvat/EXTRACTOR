...
def split_document_by_delimeters(string, regexp="[.,!?;: ]", min_word_len=1, stopwords=None):
    """
    :param string: input.txt string (text document)
...
def find_chunks(tagged, language="en"):
    """ The input.txt is a list of [token, tag]-items.
...
def find_prepositions(chunked):
    """ The input.txt is a list of [token, tag, chunk]-items.
...
def commandline(parse=Parser().parse):
    ...
    p.add_option("-O", "--tokenize",  dest="tokenize",  action="store_true", help="tokenize the input.txt")
